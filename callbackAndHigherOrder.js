...
//Another way
const contains = (arr,elem,callBack) => {
  for(let i = 0; i < arr.length; i++){
    if(elem === arr[i]){
      callBack(true)
      return
...
// CODE HERE
uniq(names, uniArr => {console.log(`The new names array with all the duplicate items removed is ${uniArr}.`)})
...
// CODE HERE
each(names, (item, indx) => {console.log('The item at index ', indx, ' is ', item)})
...
//one line of code
const getUserById = (arr,userId, callBack) => arr.forEach(elem => elem.id === userId ? callBack(elem) : null)
...
// function addingFactory(numX){
//    return function(numY){
//     return numX + numY
//   }
// }

const addingFactory = numX => numY => numX + numY
